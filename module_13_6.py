...
kb = InlineKeyboardMarkup(resize_keyboard=True)  # инициализируем клавиатуру
button_norma = InlineKeyboardButton(text='Рассчитать норму калорий', callback_data='calories')  # инициализация кнопки
button_calculate = InlineKeyboardButton(text='Формулы расчёта', callback_data='formulas')  # инициализация кнопки
kb.row(button_norma, button_calculate)  # добавление кнопок на клавиатуру
...
@dp.callback_query_handler(text='formulas')  # функция, которая выдает формулу
...
@dp.callback_query_handler(text='calories')  # теперь реагирует на текст 'calories' на кнопке
...
async def start(message):
    await message.answer('Привет! Я бот, помогающий твоему здоровью. Введи "Рассчитать"')
